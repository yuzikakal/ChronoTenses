...
export default function TextBox({ 
  ...
  return (
    <div 
      onClick={onClick}
      className="relative z-10 w-full max-w-[90%] md:max-w-[70%] bg-chrono-dark/85 backdrop-blur-md hover:bg-chrono-dark border-4 border-chrono-gold/60 rounded-[2rem] p-6 lg:p-8 2xl:p-10 cursor-pointer transition-all shadow-[0_0_30px_rgba(255,215,0,0.15)] text-md lg:text-2xl xl:text-4xl flex justify-center group"
    >
      ...
      <p className="text-chrono-text font-medium text-center leading-relaxed min-h-[80%] w-full xl:max-w-[90%] drop-shadow-sm text-base md:text-2xl lg:text-3xl 2xl:text-4xl">
        {displayedText}
      </p>

      {!isTyping && displayNext && (
        <div className="absolute bottom-4 right-8 text-chrono-gold animate-bounce">
          <span className="text-2xl md:text-3xl">{displayNext}</span>
        </div>
      )}
